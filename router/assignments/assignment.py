...
async def create_assignment(assignment:Assignments,db:Session = Depends(get_db)):
    try:
...
async def get_assignments_institute_wise(institute_id:int,db:Session = Depends(get_db)):
    assignments = ModelManager.get_data_by_institute(db.query(Assignments),Assignments,institute_id)
...
async def update_assignment(assignment_id:int,assignment:Assignments,db:Session = Depends(get_db)):
    try:
        assignment_data = db.query(Assignments).filter(Assignments.assignment_id == assignment_id).first()
...
async def delete_assignment(assignment_id:int,db:Session = Depends(get_db)):
    try:
        assignment_data = db.query(Assignments).filter(Assignments.assignment_id == assignment_id).first()
        assignment_data.is_delete = True
        db.commit()
        db.refresh(assignment_data)
        return succes_response(assignment_data)
    except Exception as e:
