...
async def create_subject_for_class(subject_data:SubjectBase,db:Session = Depends(get_db)):
    try:
...
async def get_subjects_by_class(class_id:int,db:Session = Depends(get_db)):
    class_instance = db.query(Classes).filter(Classes.class_id == class_id,Classes.is_deleted == False).first()
    if class_instance:
        subjects = db.query(Subjects).filter(Subjects.class_id == class_id,Subjects.is_deleted == False).all()
...
async def update_subject(subject_id: int, subjects: SubjectBase, db: Session = Depends(get_db)) :
