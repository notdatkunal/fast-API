...
async def create_stopage(stopage_data:StopsBase,db:Session = Depends(get_db)):
    try:
...
async def get_all_stopages(db:Session = Depends(get_db)):
    return jsonable_encoder(db.query(Stops).all())

# code for all stopages by transport id
@router.get("/get_all_stopages_by_transport/")
async def get_all_stopages(transport_id:int,db:Session = Depends(get_db)):
    data = db.query(Stops).filter(Stops.transport_id==transport_id).all()
    if data is not None:
        return jsonable_encoder(data)
    else:
        return HTTPException(status_code=404,detail="stopage not found")

# code for update stopage
@router.put("/update_stop/")
async def update_stop(stop_id:int,stop:StopsBase,db:Session = Depends(get_db)):
